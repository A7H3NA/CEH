...
````
root@kali:~# curl http://chall.2019.redpwn.net:8006/
...
When obfuscation is canceled by the online decoder, the following sources are displayed.
...
function str2blks_MD5(_0x8e5dx5) {
    nblk = ((_0x8e5dx5['length'] + 8) >> 6) + 1;
    blks = new Array(nblk * 16);
    for (i = 0; i < nblk * 16; i++) {
        blks[i] = 0
    };
    for (i = 0; i < _0x8e5dx5['length']; i++) {
        blks[i >> 2] |= _0x8e5dx5['charCodeAt'](i) << ((i % 4) * 8)
    };
    blks[i >> 2] |= 0x80 << ((i % 4) * 8);
    blks[nblk * 16 - 2] = _0x8e5dx5['length'] * 8;
    return blks
}
...
Since the password is flag, flag is redpwnctf {shazam}.
